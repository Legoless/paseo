...
describe("ProviderSnapshotManager public surface", () => {
  ...
  test("resetting a warm snapshot to loading keeps the provider selectable", async () => {
    ...
    try {
      ...
      // A warm catalog must stay selectable while settings refresh re-probes.
      const refreshed = manager.refreshSettingsSnapshot({});
      const during = await manager.getProvider({ cwd, provider: "codex", wait: false });
      expect(during.status).toBe("ready");
      expect(during.models).toHaveLength(1);
      await refreshed;
    } finally {
      manager.destroy();
    }
