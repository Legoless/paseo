...
export class ProviderSnapshotManager {
  ...
  private async resolveCatalog(
    ...
    if (
      !force &&
      catalog.result?.entry.fetchedAt &&
      Date.now() - Date.parse(catalog.result.entry.fetchedAt) >= CATALOG_STALE_TIME_MS
    ) {
      catalog.stale = true;
    }
    if (!force && (catalog.load || (catalog.result && !catalog.stale))) return catalog.load;
    if (catalog.stale) {
      catalogOptions = createFetchCatalogOptions(options.catalogScope, true);
    }
    catalog.stale = false;
  ...
  private async refreshProvider(options: {
    ...
    client: AgentClient;
    previous?: ProviderSnapshotEntry;
    ...
    try {
      ...
      if (models.length !== catalog.models.length && catalog.models.length > 0) {
        this.logger.warn(
      ...
      if (published) {
        client.setModelCatalog?.(models, catalogOptions);
        this.events.emit("catalog", provider, catalogOptions);
      }
    } catch (error) {
