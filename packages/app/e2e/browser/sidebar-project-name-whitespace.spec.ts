...
import { openProjects } from "../support/helpers/project-settings";
...
// The workspace-grouped sidebar hides a project no workspace holds, so the
// Projects list is where the loaded project shows up. The sidebar's order
// reconcile has run by then, which is what used to crash.
async function expectProjectRow(page: Page, projectKey: string): Promise<void> {
  const row = page.getByTestId(`project-row-${projectEquivalenceViewKey(projectKey)}`);
...
test("a project whose folder name ends in a space loads without crashing", async ({ page }) => {
