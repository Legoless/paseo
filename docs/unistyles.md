...
- **Web `font-family` must keep a real space glyph.** The apply path writes `--paseo-ui-font` onto every non-mono node. Android's typeface name `normal` is not a CSS family — some installed fonts use that name and draw U+0020 at width 0, so "Worked for" becomes "Workedfor". `resolveWebUiFontStack` drops `normal` and any stack whose space advance is under 1px, then uses `DEFAULT_WEB_UI_FONT_STACK`. Do not put `system-ui` alone first if you can avoid it; user-installed families can steal that name.
