...
Windows and Linux controls are fixed window chrome, outside scrolling header content. A tab rail that reaches them ends at their obstruction and shows the shared overflow fade. The project tray stays inside each workspace pane's content, with the development-build branch label at the top-left and configurable Open in editor and git actions immediately left of the Explorer toggle at the top-right, so opening and closing the pane-local dock does not move either corner.
