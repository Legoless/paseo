...
The built-in `claude` provider appends concrete model IDs from `~/.claude/settings.json` to its
[native catalog](providers.md#model-catalogs). It reads the top-level `model` field and these `env`
keys: `ANTHROPIC_MODEL`, `ANTHROPIC_SMALL_FAST_MODEL`, `ANTHROPIC_DEFAULT_OPUS_MODEL`,
`ANTHROPIC_DEFAULT_SONNET_MODEL`, and `ANTHROPIC_DEFAULT_HAIKU_MODEL`.

These IDs are passed unchanged to Claude Code, including gateway IDs that are absent from native
discovery. For custom models, specify `thinkingOptions` when the gateway supports them; Paseo no
longer invents effort levels from a familiar-looking model name. The `models` and `additionalModels`
settings above still control replacement and additions.
