...
| `titleSetByUser`     | `boolean?`                               | Set once a person renames the agent, never cleared. The MCP `update_agent` tool then leaves `title` alone and applies only its labels. Never leaves the daemon, so an agent can neither read nor clear it; absent means the title is still automatic.                                                                                                                               |
